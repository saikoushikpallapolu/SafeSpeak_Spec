...
export function setupSocketHandlers(io: Server) {
  ...
  queueManager.setOnMatchFound(({ user1Payload, user2Payload }) => {
    ...
    if (user1Payload.isSimulatedPeer) {
      ...
      io.to(user1Payload.myTag).emit('match_found', user1Payload)
    } else if (user2Payload) {
      io.to(user1Payload.myTag).emit('match_found', user1Payload)
      io.to(user2Payload.myTag).emit('match_found', user2Payload)
    }
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('join_queue', (data: MatchRequest) => {
      const enrichedReq: MatchRequest = {
        ...data,
        socketId: socket.id,
        characterTag: socket.id,
      }
      socket.data.character = data.characterId
      socket.data.tag = data.characterTag
    ...
    socket.on('join_chat_room', ({ roomId, character, tag, language }) => {
      ...
      if (!room) {
        room = {
          roomId,
          isGroup: false,
          isSimulated: roomId.includes('sim'),
          participants: new Map(),
          messages: [],
          startTime: Date.now(),
        }
        if (roomId.includes('sim')) {
